...
import { PureIcon } from '@dear-ui/core/icon'
import { Menus } from '@dear-ui/core/menu'
import { AdminWebScaffold, Link } from '@dear-ui/admin-scaffold';
import ActionAgentPage from '@dear-ui/admin-scaffold/action-agent/demo'
import ReportDemo from '@dear-ui/admin-scaffold/template-engine/for-report/demo'
import { TestFormBasic } from '@dear-ui/admin-scaffold/template-engine/for-form/demo'
import zhCH from '@dear-ui/admin-scaffold/i18n/zh-CN'
import enUS from '@dear-ui/admin-scaffold/i18n/en-US'
import VersionInfo from '@dear-ui/admin-scaffold/version.json'
...
<Playground>
  {
    () => {
      const githubLink = 'https://github.com/SANGET/@dear-ui/admin-scaffold';
...
@dear-ui/admin-scaffold 的国际化策略为：通过用户浏览器默认语言，以及右上角提供的选择器确定目标语言，获取指定目录 json 文件，包装成符合 @dear-ui 国际化策略的方式实现。
...
3. 脚手架尝试获取国际化目录下的国际化配置文件，并且导入项目当中，配合 @dear-ui 的国际化策略。

### 国际化选项配置

```js
import zhCN from '@dear-ui/admin-scaffold/i18n/zh-CN';
import enUS from '@dear-ui/admin-scaffold/i18n/en-US';
...
```js
import { AdminWebScaffold } from '@dear-ui/admin-scaffold'
